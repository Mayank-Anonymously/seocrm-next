...
import { fetchAllRoles, fetchAllUser } from "Components/slices/user/thunk";
...
import { api_is_clientdata_loading } from "Components/slices/jobs/_client/reducers";
...
const index = (props: any) => {
  ...
  const {
    ...
    serverSideLoading: state.clientFeeds.serverSideLoading,
        ...
        columns: [
          ...
          },
          {
            ...
            filterSelectOptions: [
              ...
              // { label: row.WorkType, value: row.WorkType },
            ],
          },
          {
...

          {
            ...
            sortComparator: (a: any, b: any) => {
              ...
              var dateB = new Date(a.PostDate);
              if (dateA <= dateB) {
                // u can convert date into second to compare    return -1;
                return -1;
              }
              if (dateA >= dateB) {
                return 1;
...
export const getServerSideProps = async (context: any) => {
  try {
    ...
    var filterVMS = data
      .filter((item: any) => item.accountManager == userId)
      .map((ite: any) => {
        return ite.vmsName;
      });
    ...
    const options = {
      ...
      data: JSON.stringify({ vmsIds: filterVMS }),
    };
    const fetch_client_jobs: any = axios.request(options);
    const jobs_client = await fetch_client_jobs;
    const response = Object.keys(jobs_client).map(
      (item, index) => jobs_client[item]
    );

    return { props: { clientdata: response } };
  } catch (error) {
