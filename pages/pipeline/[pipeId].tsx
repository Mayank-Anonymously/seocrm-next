...
export const getServerSideProps = async (context: any) => {
  const { pipeId } = context.query;

  var response: any = await axios.get(
    `${job_api_host}${GET_ALL_MATCHING_JOBS}`
  );
  var data: any = await response;
  var keysresp = Object.keys(data).map((ite: any) => data[ite]);

  let jobsFeeds: any[] = [];

  keysresp
    .filter((item: any) => item.jobPipeline.createdBy === pipeId)
    .map((ite: any) => jobsFeeds.push(...ite.jobsFeeds));

  return { props: { jobsFeeds: jobsFeeds } };
};
