...
import { updateLocation } from "Components/slices/location/thunk";
import { baseURL } from "Components/helpers/url_helper";
import { useRouter } from "next/router";

const index = ({ locationId }: any) => {
  ...
  const { category, subcat, sellocation } = useSelector((state: any) => ({
    category: state.CategorySlice.categorydata,
    subcat: state.CategorySlice.subcat,
    sellocation: state.location.selectedlocation,
  }));
  const formik: any = useFormik({
    ...
    validationSchema: Yup.object({
      // locationName: Yup.array().required("Required"),
    }),
    onSubmit: (values: any) => {
      dispatch(updateLocation(values, locationId, router));
    },
  });

  useEffect(() => {
    handleFetchLocation(sellocation.locationName);
  }, []);
                    ...
                    <Col>
                      ...
                      <div>
                        <Form.Label htmlFor="radius" className="form-label">
                          Map Radius (CURRENT RADIUS : {sellocation.radius})
                        </Form.Label>
                        <Form.Control
                          name="radius"
                          id="radius"
