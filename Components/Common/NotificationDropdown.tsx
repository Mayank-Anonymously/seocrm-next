...
import { job_api_host } from "Components/helpers/url_helper";
...
const NotificationDropdown = () => {
  ...
  useEffect(() => {
    if (!listening) {
      eventSource = new EventSource(`${job_api_host}/pipeline/stream`);
      eventSource.onopen = (event: any) => {};
      eventSource.onmessage = (event: any) => {
        const newData = event.data;
        setData((oldData: string[]) => [...oldData, newData]);
      };
  ...
  const notidata = data.map((jsonString, index) => {
    const parsedData = JSON.parse(jsonString);
    // console.log("Parsed Data:", parsedData);

    const processFilteredData = (jsonString: any) => {
      const parsedData = JSON.parse(jsonString);
      return parsedData.filter(
        (item: any) => item.jobPipeline.createdBy === userID.id
      );
    };

    data.forEach((jsonString, index) => {
      filteredData = filteredData.concat(processFilteredData(jsonString));
    });

    filteredDataLength = filteredData.length;

    // Optionally, you can return JSX for the filtered items
    return filteredData.map((item: any, itemIndex: any) => (
      <div key={itemIndex}>
        {item.jobPipeline.candidateName} : {item.jobsFeeds.length}{" "}
      </div>
    ));
  });
  return (
      ...
      >
        ...
        >
          ...
          <span className="position-absolute topbar-badge fs-10 translate-middle badge rounded-pill bg-danger">
            <span className="notification-badge">{filteredDataLength}</span>
            ...
            <SimpleBar style={{ maxHeight: "300px" }} className="pe-2">
              ...
              <div className="text-reset notification-item d-block dropdown-item position-relative unread-message">
                <div className="d-flex">
                  ...
                  <div className="flex-grow-1 notification-container">
                            ...
                            <div className="d-flex align-items-center">
                              ...
                              <h6 className="mt-0 fs-14 mb-2 lh-base">
                                ...
                                <span className="text-secondary">
                                  {item.jobsFeeds.length} &nbsp;
                                </span>
                                job match found for your pipeline of &nbsp;
                                <span className="text-secondary">
                                  {item.jobPipeline.firstName}
                                </span>
                              </h6>
                            </div>
                          </a>
            ...
            <span>
              ...
              <div className="text-reset notification-item d-block dropdown-item position-relative unread-message">
                <div className="d-flex">
                  ...
                  <div className="flex-grow-1">
                    <Link href="#!" className="stretched-link">
                      <h6 className="mt-0 mb-2 fs-14 lh-base">
