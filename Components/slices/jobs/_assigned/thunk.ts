...
export const getAssignedJobsToMe = () => async (dispatch: any) => {
  try {
    if (localStorage.getItem("authUser")) {
      ...
      // if (role[0].role === "TEAMLEAD") {
      //   dispatch(getJobsAssignedToTL(obj.id));

      //   var feeds: any = [];

      //   data.map((item: any, index: any) => {
      //     item.jobsFeedsSet.forEach((element: any) => feeds.push(element));
      //   });
      //   console.log(feeds);
      //   dispatch(assigned_feed_to_me(feeds));
      // } else {
      //   var feeds: any = [];
...
export const getJobsAssignedToTL = (tlId: any) => async (dispatch: any) => {
  try {
    if (localStorage.getItem("currentrole")) {
      const role = JSON.parse(localStorage.getItem("currentrole") || "");
      const options = {
        url: `${job_api_host}${GET_ASSIGNED_JOBS_TO_TL}/${tlId}`,
        method: "POST",
        headers: { "Content-Type": "application/json" },
        data: "{}",
      };
      ...
      dispatch(assigned_to_recruiter(feeds));
      return data;
    }
  } catch (error) {
    dispatch(api_is_assigndata_error(error));
